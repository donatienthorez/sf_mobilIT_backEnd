adminModule.service('countryGuideRequest', CountryGuideRequest);

CountryGuideRequest.$inject = ['$http'];

function CountryGuideRequest($http) {
    var ctrl = this;
    ...
    function addCategory() {
        return $http({
            method: 'POST',
            url: Routing.generate(
                'api_categories_add_country_guide_category'
            )
        }).then(function (result) {
            return result.data;
        });
    ...
    function changeGuideStatus() {
        return $http({
            method: 'PUT',
            url: Routing.generate(
                'api_country_guides_change_status'
            )
        }).then(function (result) {
            return result.data;
        });
